...
R: Com um tratamento de exeção seria possivel evitar o erro,ou fazer alguma verificação para quando o usuario digitasse um id que não exista ou algum tipo diferente do esperado como uma string por exemplo, aparecer uma caixa na tela dizendo que o id não foi encontrado e para digitar apenas numeros. Poderia mas teria que corrigir todos os outros metodos onde ele é chamado, a desvantagem é como eu disse, teria que corrigir e provavelmete criar outros metodos para garantir o funcionamento, e a vantagem é que o erro seria corrigido.
